...
UI::SceneWindow::SceneWindow(std::shared_ptr<Utils::CameraManager> camera_manager, 
    ...
    , aruco_window_(aruco_window)
    , current_dictionary_type_(cv::aruco::DICT_4X4_50)
    , current_selected_marker_id_(0)
    ...
    if (now - last_stats_update_ >= std::chrono::seconds(1))
    {
        for (auto& stat : marker_statistics_)
        {
            auto duration = std::chrono::duration_cast<std::chrono::seconds>(
                stat.second.last_detection - stat.second.first_detection).count();
...
void UI::SceneWindow::render_detected_arucos()
{
    ...
    // Display the image
    ImVec2 image_size(display_frame_rgba.cols, display_frame_rgba.rows);
...
void UI::SceneWindow::render_statistics()
{
    ...
    for (const auto& stat_pair : marker_statistics_)
    {
        ...
        {
            auto duration = std::chrono::duration_cast<std::chrono::seconds>(
                stat.last_detection - stat.first_detection).count();
